...
Enhanced with file extension validation
"""
...
class CSVRegressionTester:
    ...
    def _find_reference_file(self, base_name: str) -> Optional[Path]:
        """Find reference file with any supported extension"""
    ...
    def _find_generated_file(self, base_name: str) -> Optional[Path]:
        """Find generated file with any supported extension"""
        supported_extensions = ['.csv', '.xlsx', '.xls']
    ...
    def test_file(self, file_name: str) -> ValidationReport:
        """Test a single CSV file against its reference - ENHANCED WITH EXTENSION VALIDATION"""
        ...
        try:
            # Find reference and generated files
            ...
            if not ref_file:
                report.add_issue(ValidationIssue(
                    ...
                    severity=ValidationResult.FAIL,
                    message=f"Reference file not found for base name: {base_name} (searched extensions: .csv, .xlsx, .xls)"
                ))
                return report
            ...
            if not gen_file:
                report.add_issue(ValidationIssue(
                    ...
                    severity=ValidationResult.FAIL,
                    message=f"Generated file not found for base name: {base_name} (searched extensions: .csv, .xlsx, .xls)"
                ))
                return report
            
            # NEW: Check file extension compatibility
